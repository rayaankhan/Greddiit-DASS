...
function OtherProfilePage() {
    ...
    useEffect(() => {
        axios.post("http://localhost:9002/getprevvaluesother",{token : tok, userName: username})
        .then((res) => {
    ...
    useEffect(() => {
      axios.post("http://localhost:9002/checkfollowers",{token : tok , userName : username})
      .then((res)=>{
    ...
    // const [areYouFollowing, setAreYouFollowing] = useState('Follow')

    const toFollow = () => {
        if(buttonchange === false){
            axios.post("http://localhost:9002/updateunfollowers", {token: tok, userName: username, followers: profileDetails.followersNumber-1})
        .then((res) => {
        if(res.data.message === 1)
        {
        //   alert("updated followers")
        ...
        axios.post("http://localhost:9002/updatefollowers", {token: tok, userName: username, followers: profileDetails.followersNumber+1})
        .then((res) => {
        if(res.data.message === 1)
        {
        //   alert("updated followers")
          ...
          <div className="rounded-top text-white d-flex flex-row" style={{backgroundColor: "#000", height:"200px"}}>
            <div className="ms-4 mt-5 d-flex flex-column" style={{width: "150px"}}>
              ...
              {/* <div className="d-flex pt-1">
                <button type="button" className="btn btn-primary flex-grow-1" style={{zIndex: "1", display: "inline-block"}}>Log Out</button>
              </div> */}
              <div className="d-flex pt-1">
              {buttonchange === true ? <button style={{zIndex: "1"}} onClick={toFollow}>Follow</button> : <button style={{zIndex: "1"}} onClick={toFollow}>Unfollow</button>}
                {/* <button type="button" className="btn btn-primary flex-grow-1" onClick={toFollow} style={{zIndex: "1", display: "inline-block"}}>{areYouFollowing}</button> */}
              </div>
            </div>
